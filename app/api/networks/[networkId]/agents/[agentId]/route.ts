...
export async function DELETE(
  request: NextRequest,
  context: { params: Promise<{ networkId: string; agentId: string }> }
) {
  try {
...
// GET /api/networks/[networkId]/agents/[agentId] - Get a specific agent in a network
export async function GET(
  ...
  try {
    ...
    return NextResponse.json(existingNetworkAgent);
  } catch (error) {
    console.error('Error in GET /api/networks/[networkId]/agents/[agentId]:', error);
    return NextResponse.json(
      { error: 'Internal server error' },
      { status: 500 }
    );
