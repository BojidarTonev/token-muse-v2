...
const roleOptions = [
  {
    value: "primary",
    label: "Primary Agent",
    description: "The main agent that coordinates the network",
  },
  {
    value: "processor",
    label: "Data Processor",
    description: "Processes and transforms data for other agents",
  },
  {
    value: "analyzer",
    label: "Analyzer",
    description: "Analyzes and extracts insights from data",
  },
  {
    value: "creator",
    label: "Content Creator",
    description: "Generates creative content based on inputs",
  },
  {
    value: "validator",
    label: "Validator",
    description: "Validates outputs from other agents",
  },
];
...
export default function CreateNetworkPage() {
  ...
  // UI state
  const [activeStep, setActiveStep] = useState<"info" | "agents" | "review">(
    "info"
  );
    ...
    const fetchAgents = async () => {
      try {
        ...
        if (isAuthenticated && publicKey) {
          ...
          const userResponse = await fetchWithPublicKey(
            "/api/agents",
            publicKey
          );
  ...
  const createNetwork = async () => {
    ...
    try {
      ...
      for (const agent of selectedAgents) {
        console.log(
          `Adding agent ${agent.id} with role ${agent.role} to network ${networkId}`
        );
        ...
        if (!agentResponse.ok) {
          ...
          console.error(
            `Failed to add agent ${agent.id} to network:`,
            errorData
          );
          throw new Error(
            `Failed to add agent ${agent.id} to network: ${
      ...
      window.location.href = `/network?created=true`;
    } catch (err) {
            ...
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
              ...
              <div>
                ...
                {selectedAgents.length === 0 ? (
                  <div className="text-center py-10 border border-dashed border-border rounded-lg">
                    <p className="text-foreground/70">
                      Add agents from the left panel to include them in your
                      network.
                    </p>
                  </div>
                ) : (
                  <div className="space-y-3 max-h-[500px] overflow-y-auto pr-2">
                    {selectedAgents.map((agent) =>
                      renderAgentCard(agent, true, false)
                    )}
                  </div>
                )}
              </div>
          ...
          <div className="space-y-6">
            ...
            <div className="feature-card">
              <h3 className="text-lg font-medium mb-4">
                Network Agents ({selectedAgents.length})
              </h3>
                        ...
                        <div className="flex items-center justify-between">
                          <div>
                            ...
                            <div className="text-xs text-foreground/70">
                              {roleOptions.find((r) => r.value === agent.role)
                                ?.label || agent.role}
                            </div>
                          </div>
                ...
                <div className="absolute inset-0 flex items-center justify-center">
                  <div className="relative w-full h-full">
                    ...
                    {selectedAgents.map((agent, index) => {
                      ...
                      const angle =
                        (index / selectedAgents.length) * Math.PI * 2;
            ...
            <div className="flex flex-col md:flex-row justify-between items-start md:items-center mb-8">
              <div>
                ...
                <p className="text-foreground/70 max-w-xl">
                  Build a powerful network by connecting multiple AI agents to
            ...
            <div className="feature-card mb-8">
              <div className="flex items-center gap-4 mb-6">
                ...
                <div>
                  ...
                  <p className="text-foreground/70">
                    Follow the steps below to create your custom agent network
                  </p>
                </div>
              </div>
