...
import { Plus, Zap, Users, Sparkles, Star, Brain, Cpu, Rocket, Database, Link as LinkIcon, ChevronDown, ChevronUp } from "lucide-react";
...
export default function NetworkPage() {
  ...
  }, [viewMode, isAuthenticated, publicKey]);
  
  // Function to fetch user networks
  const fetchUserNetworks = async () => {
    ...
    try {
      ...
      const response = await fetchWithPublicKey('/api/networks', publicKey);
              ...
              <section className="mb-16">
                {!isAuthenticated ? (
                  ...
                  <div className="feature-card text-center py-16 flex flex-col items-center">
                    ...
                    <h2 className="text-2xl font-semibold mb-3">No Networks Yet</h2>
                    <p className="text-foreground/70 max-w-md mx-auto mb-8 text-center">
                      You haven&apos;t created any networks yet. Start by creating your first network to connect multiple agents together.
                    </p>
                        ...
                        <Link href={`/network/${network.id}`} key={network.id}>
                          <div className="feature-card h-full hover:border-primary/50 transition-colors cursor-pointer">
                            <div className="flex items-center gap-4 mb-4">
                              ...
                              <h3 className="text-lg font-medium truncate">{network.name}</h3>
                            </div>
                            ...
                            <div className="mt-auto pt-4 flex justify-between items-center text-sm text-foreground/60">
                              <div>Created: {new Date(network.created_at).toLocaleDateString()}</div>
